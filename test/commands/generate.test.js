...
describe("CLI Commands - generate (src/cli/commands/generate.js)", function () {
  ...
  // -------------------------------------------------------------------
  // --db-manager flag integration (Requirements: 1.1, 1.2, 1.3, 1.4, 8.2)
  // -------------------------------------------------------------------
  describe("--db-manager flag", function () {
    const DB_MANAGER_FILES = [
      "routes/database.js",
    ...
    it("should include DB Manager files when --db-manager is passed with a SQL adapter (Req 1.1)", async function () {
      const schemaPath = writeSchemaFile(tmpDir, { adapter: "postgres" });
      const ctx = new OutputContext({ json: true });

      await generateCmd(
        { from: schemaPath, "db-manager": true },
        {
          yes: false,
          json: true,
          dryRun: false,
      ...
      for (const expected of DB_MANAGER_FILES) {
        assert.ok(
          paths.includes(expected),
          `Should include DB Manager file: ${expected}`,
        );
      }
    });

    it("should list DB Manager files as planned with --db-manager --dry-run (Req 1.3)", async function () {
      const schemaPath = writeSchemaFile(tmpDir, { adapter: "sqlite3" });
      const ctx = new OutputContext({ json: true });
      ...
      for (const expected of DB_MANAGER_FILES) {
        assert.ok(
          paths.includes(expected),
          `Should list DB Manager file as planned: ${expected}`,
        );
      }

      // All DB Manager files should have "planned" status
      const dbmFiles = result.files.filter((f) =>
        DB_MANAGER_FILES.includes(f.path),
      );
      for (const f of dbmFiles) {
        assert.strictEqual(
          f.status,
          "planned",
          `${f.path} should have planned status in dry-run`,
        );
      }

      // Verify no DB Manager files were written to disk
      assert.ok(
        !fs.existsSync(path.join(tmpDir, "routes/database.js")),
        "routes/database.js should NOT exist in dry-run mode",
      );
      assert.ok(
        !fs.existsSync(path.join(tmpDir, "views/db-manager")),
        "views/db-manager/ should NOT exist in dry-run mode",
      );
    });

    it("should include DB Manager files in JSON output with --db-manager --json (Req 1.4)", async function () {
      const schemaPath = writeSchemaFile(tmpDir, { adapter: "mysql" });
      const ctx = new OutputContext({ json: true });
      ...
      for (const expected of DB_MANAGER_FILES) {
        assert.ok(
          paths.includes(expected),
          `JSON output should include DB Manager file: ${expected}`,
        );
      }

      // Each DB Manager file should have a valid status
      const dbmFiles = result.files.filter((f) =>
        DB_MANAGER_FILES.includes(f.path),
      );
      for (const f of dbmFiles) {
        assert.ok(
          ["created", "overwritten", "unchanged", "planned"].includes(f.status),
          `${f.path} should have a valid status, got: ${f.status}`,
        );
      }

      // Verify the result is valid JSON when serialized
      const jsonStr = JSON.stringify(result);
      assert.doesNotThrow(
        () => JSON.parse(jsonStr),
        "Result should be serializable to valid JSON",
    ...
    it("should not include DB Manager files and should log warning for NoSQL adapter (Req 8.2)", async function () {
      const schemaPath = writeSchemaFile(tmpDir, { adapter: "mongodb" });
      // Use non-json mode to capture log output for warning check
      const logs = [];
      const ctx = {
      ...
      const warningLogged = logs.some((msg) => msg.includes("warning"));
      assert.ok(warningLogged, "Should log a warning for NoSQL adapter");

      // Verify no DB Manager files in the dry-run output
      const plannedLogs = logs.filter(
        (msg) =>
          msg.includes("routes/database.js") ||
          msg.includes("middleware/db-manager-auth.js") ||
          msg.includes("views/db-manager/"),
      );
      assert.strictEqual(
        plannedLogs.length,
        0,
        "Should not list any DB Manager files for NoSQL adapter",
      );
    });

    it("should NOT include DB Manager files when --db-manager is not passed (Req 1.2)", async function () {
      const schemaPath = writeSchemaFile(tmpDir, { adapter: "postgres" });
      const ctx = new OutputContext({ json: true });
      ...
      for (const dbmFile of DB_MANAGER_FILES) {
        assert.ok(
          !paths.includes(dbmFile),
          `Should NOT include DB Manager file when flag not passed: ${dbmFile}`,
        );
      }
    });
  });
