...
const { where } = require("../../src/sqlite3/db");
...
/**
 * Count the number of ? bind parameters in a query string.
 */
...
describe("Feature: database-adapter-standardization, Property 1: Filter translation produces valid where clauses (SQLite3)", function () {
  /**
   * **Validates: Requirements 1.2**
   *
  ...
  it("should produce query with bind parameter count matching value array length", function () {
    fc.assert(
      fc.property(arbFilterArray, (filterArray) => {
        ...
        // Bind parameter count must match value array length
        const bindCount = countBindParams(result.query);
        assert.strictEqual(
          bindCount,
          result.value.length,
    ...
    fc.assert(
      fc.property(arbFilterArray, (filterArray) => {
        ...
        for (const andGroup of filterArray) {
          for (const condition of andGroup) {
            const op = condition[1];
            // The operator should appear in the query (case-insensitive check)
            assert.ok(
